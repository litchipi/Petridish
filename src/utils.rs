...
impl MeanCompute {
    ...
    pub fn add_el(&mut self, element: f64, weight: f64) -> f64 {
        let res =
            ((self.result * self.sumweights) + (element * weight)) / (self.sumweights + weight);
        self.result = res;
...
impl MeanComputeVec {
    ...
    pub fn add_el(&mut self, element: &Vec<f64>, weight: f64) {
        for i in 0..element.len() {
            let res = if weight == 0.0 {
                0.0
            } else {
                ((self.result[i] * self.sumweights) + (element[i] * weight))
                    / (self.sumweights + weight)
            };
            self.result[i] = res;
        }
    ...
    pub fn add_el(&mut self, element: &Vec<f64>) {
        ...
        for i in 0..self.result.len() {
            ...
            if self.result[i] == f64::NAN {
                println!(
                    "{} {} {} {}",
                    self.count, self.mean[i], self.ex2[i], self.result[i]
                );
