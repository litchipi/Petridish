...
impl<T: Cell> GenalgoMethod<T> for DarwinMethod<T>{
    ...
    fn init_method(&mut self, bestgen: &Genome, nb_cells: u32, nb_elites: u32, res: &mut Vec<Genome>) -> Result<(), Errcode>{
        if bestgen.len() == 0 {
            ...
            return Ok(self.__init_generate_population_from_bestgen(bestgen.clone(), nb_cells, nb_elites, res))
        }
    }
...
impl<T: Cell> DarwinMethod<T>{
    ...
    fn __init_generate_population_from_bestgen(&mut self, bestgen: Genome, nb_cells: u32, nb_elites: u32, genomes: &mut Vec<Genome>){
